...
class LocalNotificationService {
  int id = 0;
  // Sau khi init xong sẽ có thông tin này
  String initialRoute = NotificationRoute.home.name;
  bool didNotificationLaunchApp = false;
  String? selectedNotificationPayload;

  BuildContext get context => Get.context!;

  // Check permission cho notification
  ...
  Future<void> init() async {
    ...
    if (notificationAppLaunchDetails?.didNotificationLaunchApp ?? false) {
      ...
      updateInitRouteFromPayload(selectedNotificationPayload);
    }
    ...
    final LinuxInitializationSettings initializationSettingsLinux =
      ...
      defaultIcon: AssetsLinuxIcon('assets/base/notification/app_icon.png'),
    );
  ...
  void updateInitRouteFromPayload(String? payload) {
    if (payload == null) {
      return;
    }
    if (payload.contains('product')) {
      initialRoute = NotificationRoute.product.name;
    }
    if (payload.contains('detail')) {
      initialRoute = NotificationRoute.detail.name;
    }
  }
  ...
  // Đang mở app thì lại ấn vào notification
